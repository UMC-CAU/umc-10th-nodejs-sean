...
export const addUser = async (data: any): Promise<number | null> => {
  ...
  try {
    ...
    const [result] = await pool.query<ResultSetHeader>(
      `INSERT INTO user (email, password, name, nickname, gender, birth_date, address, detail_address, phone_number) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);`,
      [
        data.email,
        data.password,
...
export const setPreference = async (userId: number, foodCategoryId: number): Promise<void> => {
  ...
  try {
    ...
    throw new Error(`선호음식 추가 중 오류가 발생했습니다: ${err}`);
  } finally {
    conn.release();
  }
};
...
export const getUserPreferencesByUserId = async (userId: number): Promise<any[]> => {
  ...
  try {
    ...
    throw new Error(`선호음식 조회 중 오류가 발생했습니다: ${err}`);
  } finally {
    conn.release();
  }
};